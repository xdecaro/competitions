...
A consumer such as People may boot `com_competitions` at runtime, obtain `getCoreIntegrationService()`, register the returned Competitions capabilities into an in-memory Core `CapabilityRegistry`, require `competitions.people_history` v1, and only then call `getPersonHistoryService()`.
