...
$approvalLabels = [
    'pending' => 'COM_DECARODCL_PLAYER_PENDING',
    'approved' => 'COM_DECARODCL_PLAYER_APPROVED',
    'rejected' => 'COM_DECARODCL_PLAYER_REJECTED',
];
$approvalClasses = ['pending' => 'bg-warning text-dark', 'approved' => 'bg-success', 'rejected' => 'bg-danger'];
?>
<form action="<?= Route::_('index.php?option=com_decarodcl&view=players'); ?>" method="post" name="adminForm" id="adminForm" class="dcl-admin">
    <div class="dcl-filterbar">
        <div class="dcl-filterbar__search">
            ...
            <input type="search" name="filter_search" id="filter_search" value="<?= $this->escape($this->state->get('filter.search')); ?>" placeholder="<?= Text::_('COM_DECARODCL_SEARCH_PLAYERS'); ?>">
        </div>
        <select name="filter_nationality_code" onchange="this.form.submit()">
            <option value=""><?= Text::_('COM_DECARODCL_FILTER_ALL_NATIONALITIES'); ?></option>
        ...
        <a class="btn btn-secondary" href="<?= Route::_('index.php?option=com_decarodcl&view=players'); ?>"><?= Text::_('JSEARCH_FILTER_CLEAR'); ?></a>
    </div>
    ...
    <div class="table-responsive">
        <table class="table table-striped align-middle dcl-responsive-table">
            <thead><tr>
                ...
                <th><?= HTMLHelper::_('searchtools.sort', 'COM_DECARODCL_FIELD_PLAYER', 'a.last_name', $listDirn, $listOrder); ?></th>
                <th><?= HTMLHelper::_('searchtools.sort', 'COM_DECARODCL_FIELD_NATIONALITY', 'c.name', $listDirn, $listOrder); ?></th>
                <th><?= HTMLHelper::_('searchtools.sort', 'COM_DECARODCL_FIELD_BIRTH_DATE', 'a.birth_date', $listDirn, $listOrder); ?></th>
                <th><?= HTMLHelper::_('searchtools.sort', 'COM_DECARODCL_FIELD_APPROVAL_STATUS', 'a.approval_status', $listDirn, $listOrder); ?></th>
                <th class="text-center"><?= Text::_('COM_DECARODCL_ROSTERS'); ?></th>
            ...
            <?php foreach ($this->items as $i => $item) : ?>
                <tr>
                    ...
                    <td data-label="<?= Text::_('COM_DECARODCL_FIELD_PLAYER'); ?>">
                        <a class="fw-semibold" href="<?= Route::_('index.php?option=com_decarodcl&task=player.edit&id=' . (int) $item->id); ?>"><?= $this->escape($item->last_name . ' ' . $item->first_name); ?></a>
                        <?php if ($item->external_ref) : ?><div class="small text-muted"><?= $this->escape($item->external_ref); ?></div><?php endif; ?>
                    </td>
                    <td data-label="<?= Text::_('COM_DECARODCL_FIELD_NATIONALITY'); ?>"><?= $this->escape($item->nationality_name ?: ($item->nationality_code ?: '—')); ?></td>
                    <td data-label="<?= Text::_('COM_DECARODCL_FIELD_BIRTH_DATE'); ?>"><?= $this->escape($item->birth_date ?: '—'); ?></td>
                    <td data-label="<?= Text::_('COM_DECARODCL_FIELD_APPROVAL_STATUS'); ?>"><span class="badge <?= $approvalClasses[$item->approval_status] ?? 'bg-secondary'; ?>"><?= Text::_($approvalLabels[$item->approval_status] ?? 'COM_DECARODCL_PLAYER_PENDING'); ?></span></td>
                    <td class="text-center" data-label="<?= Text::_('COM_DECARODCL_ROSTERS'); ?>"><?= (int) $item->rosters_count; ?></td>
            ...
            <?php endforeach; ?>
            <?php if (!$this->items) : ?><tr><td colspan="8" class="text-center py-5 text-muted"><?= Text::_('COM_DECARODCL_NO_PLAYERS'); ?></td></tr><?php endif; ?>
            </tbody>
        </table>
    </div>
