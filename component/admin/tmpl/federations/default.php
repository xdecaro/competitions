...
?>
<form action="<?= Route::_('index.php?option=com_decarodcl&view=federations'); ?>" method="post" name="adminForm" id="adminForm" class="dcl-admin">
    <div class="dcl-filterbar">
        <div class="dcl-filterbar__search">
            ...
            <input type="search" name="filter_search" id="filter_search" value="<?= $this->escape($this->state->get('filter.search')); ?>" placeholder="<?= Text::_('COM_DECARODCL_SEARCH_FEDERATIONS'); ?>">
        </div>
        ...
        <a class="btn btn-secondary" href="<?= Route::_('index.php?option=com_decarodcl&view=federations'); ?>"><?= Text::_('JSEARCH_FILTER_CLEAR'); ?></a>
    </div>
        ...
        <table class="table table-striped align-middle">
            <thead>
                <tr>
                    ...
                    <th><?= HTMLHelper::_('searchtools.sort', 'COM_DECARODCL_FIELD_SHORT_NAME', 'a.short_name', $listDirn, $listOrder); ?></th>
                    <th><?= HTMLHelper::_('searchtools.sort', 'COM_DECARODCL_FIELD_COUNTRY', 'c.name', $listDirn, $listOrder); ?></th>
                    <th><?= Text::_('COM_DECARODCL_FIELD_WEBSITE'); ?></th>
            ...
            <?php foreach ($this->items as $i => $item) : ?>
                <tr>
                    ...
                    <td>
                        <a class="fw-semibold" href="<?= Route::_('index.php?option=com_decarodcl&task=federation.edit&id=' . (int) $item->id); ?>">
                    ...
                    <td>
                        <?php if ($item->website) : ?>
                            <a href="<?= $this->escape($item->website); ?>" target="_blank" rel="noopener noreferrer"><?= Text::_('COM_DECARODCL_OPEN_WEBSITE'); ?></a>
                        <?php else : ?>—<?php endif; ?>
                    </td>
            ...
            <?php endforeach; ?>
            <?php if (!$this->items) : ?>
                <tr><td colspan="7" class="text-center py-5 text-muted"><?= Text::_('COM_DECARODCL_NO_FEDERATIONS'); ?></td></tr>
            <?php endif; ?>
            </tbody>
        </table>
    </div>
