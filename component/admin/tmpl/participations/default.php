...
$canChange = Factory::getApplication()->getIdentity()->authorise('core.edit.state', 'com_decarodcl');
$listOrder = $this->escape($this->state->get('list.ordering'));
$listDirn = $this->escape($this->state->get('list.direction'));

$statusLabels = [
    'draft' => ['COM_DECARODCL_PARTICIPATION_DRAFT', 'bg-secondary'],
    'submitted' => ['COM_DECARODCL_PARTICIPATION_SUBMITTED', 'bg-info text-dark'],
    'approved' => ['COM_DECARODCL_PARTICIPATION_APPROVED', 'bg-success'],
    'rejected' => ['COM_DECARODCL_PARTICIPATION_REJECTED', 'bg-danger'],
];
?>
<form action="<?= Route::_('index.php?option=com_decarodcl&view=participations'); ?>" method="post" name="adminForm" id="adminForm" class="dcl-admin">
    <div class="dcl-filterbar dcl-filterbar--wide">
        <div class="dcl-filterbar__search">
            ...
            <input
                ...
                placeholder="<?= Text::_('COM_DECARODCL_SEARCH_PARTICIPATIONS'); ?>"
            >
        </div>

        <select name="filter_tournament_id" onchange="this.form.submit()">
            <option value="0"><?= Text::_('COM_DECARODCL_FILTER_ALL_TOURNAMENTS'); ?></option>
        ...
        <select name="filter_status" onchange="this.form.submit()">
            <option value=""><?= Text::_('COM_DECARODCL_FILTER_ALL_PARTICIPATION_STATUSES'); ?></option>
        ...
        <a class="btn btn-secondary" href="<?= Route::_('index.php?option=com_decarodcl&view=participations'); ?>"><?= Text::_('JSEARCH_FILTER_CLEAR'); ?></a>
    </div>
        ...
        <table class="table table-striped align-middle dcl-responsive-table">
            <thead>
                <tr>
                    ...
                    <th><?= HTMLHelper::_('searchtools.sort', 'COM_DECARODCL_FIELD_SEASON', 's.name', $listDirn, $listOrder); ?></th>
                    <th><?= HTMLHelper::_('searchtools.sort', 'COM_DECARODCL_FIELD_PARTICIPATION_STATUS', 'a.status', $listDirn, $listOrder); ?></th>
                    <th class="d-none d-xl-table-cell"><?= HTMLHelper::_('searchtools.sort', 'COM_DECARODCL_FIELD_SUBMITTED_AT', 'a.submitted_at', $listDirn, $listOrder); ?></th>
                    <th class="d-none d-xl-table-cell"><?= HTMLHelper::_('searchtools.sort', 'COM_DECARODCL_FIELD_REVIEWED_AT', 'a.reviewed_at', $listDirn, $listOrder); ?></th>
            ...
            <?php foreach ($this->items as $i => $item) : ?>
                <?php $status = $statusLabels[$item->status] ?? ['COM_DECARODCL_PARTICIPATION_DRAFT', 'bg-secondary']; ?>
                <tr>
                    <td class="text-center dcl-responsive-table__check"><?= HTMLHelper::_('grid.id', $i, $item->id); ?></td>
                    <td data-label="<?= Text::_('COM_DECARODCL_FIELD_TEAM'); ?>">
                        <a class="fw-semibold" href="<?= Route::_('index.php?option=com_decarodcl&task=participation.edit&id=' . (int) $item->id); ?>">
                    ...
                    <td data-label="<?= Text::_('COM_DECARODCL_FIELD_SEASON'); ?>">
                        <div><?= $this->escape($item->tournament_name ?: '—'); ?></div>
                        <div class="small text-muted">
                            <?= $this->escape(($item->season_name ?: '—') . ($item->season_year ? ' · ' . $item->season_year : '')); ?>
                        </div>
                    </td>
                    <td data-label="<?= Text::_('COM_DECARODCL_FIELD_PARTICIPATION_STATUS'); ?>"><span class="badge <?= $status[1]; ?>"><?= Text::_($status[0]); ?></span></td>
                    <td class="d-none d-xl-table-cell" data-label="<?= Text::_('COM_DECARODCL_FIELD_SUBMITTED_AT'); ?>">
                        <?= $item->submitted_at ? HTMLHelper::_('date', $item->submitted_at, Text::_('DATE_FORMAT_LC4')) : '—'; ?>
                    </td>
                    <td class="d-none d-xl-table-cell" data-label="<?= Text::_('COM_DECARODCL_FIELD_REVIEWED_AT'); ?>">
            ...
            <?php if (!$this->items) : ?>
                <tr>
                    <td colspan="8" class="text-center py-5 text-muted"><?= Text::_('COM_DECARODCL_NO_PARTICIPATIONS'); ?></td>
                </tr>
            <?php endif; ?>
            </tbody>
        </table>
