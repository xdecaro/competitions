...
$canChange = \Joomla\CMS\Factory::getApplication()->getIdentity()->authorise('core.edit.state', 'com_xdecarocompetitions');
...
?>
<form action="<?= Route::_('index.php?option=com_xdecarocompetitions&view=teams'); ?>" method="post" name="adminForm" id="adminForm" class="competitions-admin">
    <div class="competitions-filterbar competitions-filterbar--wide">
        <div class="competitions-filterbar__search">
            ...
            <input
                ...
                placeholder="<?= Text::_('COM_XDECAROCOMPETITIONS_SEARCH_TEAMS'); ?>"
            >
        </div>

        <select name="filter_team_type" onchange="this.form.submit()">
            <option value=""><?= Text::_('COM_XDECAROCOMPETITIONS_FILTER_ALL_TEAM_TYPES'); ?></option>
        ...
        <select name="filter_federation_id" onchange="this.form.submit()">
            <option value="0"><?= Text::_('COM_XDECAROCOMPETITIONS_FILTER_ALL_FEDERATIONS'); ?></option>
        ...
        <a class="btn btn-secondary" href="<?= Route::_('index.php?option=com_xdecarocompetitions&view=teams'); ?>"><?= Text::_('JSEARCH_FILTER_CLEAR'); ?></a>
    </div>
        ...
        <table class="table table-striped align-middle competitions-responsive-table">
            <thead>
                <tr>
                    ...
                    <th><?= HTMLHelper::_('searchtools.sort', 'COM_XDECAROCOMPETITIONS_FIELD_FEDERATION', 'f.name', $listDirn, $listOrder); ?></th>
                    <th class="d-none d-xl-table-cell"><?= HTMLHelper::_('searchtools.sort', 'COM_XDECAROCOMPETITIONS_FIELD_TEAM_MANAGER', 'u.name', $listDirn, $listOrder); ?></th>
                    <th><?= HTMLHelper::_('searchtools.sort', 'COM_XDECAROCOMPETITIONS_FIELD_APPROVAL_STATUS', 'a.approval_status', $listDirn, $listOrder); ?></th>
                    <th class="text-center d-none d-lg-table-cell"><?= HTMLHelper::_('searchtools.sort', 'COM_XDECAROCOMPETITIONS_PARTICIPATIONS', 'participations_count', $listDirn, $listOrder); ?></th>
            ...
            <?php foreach ($this->items as $i => $item) : ?>
                ...
                <tr>
                    ...
                    <td data-label="<?= Text::_('COM_XDECAROCOMPETITIONS_FIELD_NAME'); ?>">
                        <a class="fw-semibold" href="<?= Route::_('index.php?option=com_xdecarocompetitions&task=team.edit&id=' . (int) $item->id); ?>">
                    ...
                    <td data-label="<?= Text::_('COM_XDECAROCOMPETITIONS_FIELD_APPROVAL_STATUS'); ?>"><span class="badge <?= $approval[1]; ?>"><?= Text::_($approval[0]); ?></span></td>
                    <td class="text-center d-none d-lg-table-cell" data-label="<?= Text::_('COM_XDECAROCOMPETITIONS_PARTICIPATIONS'); ?>">
                        <a href="<?= Route::_('index.php?option=com_xdecarocompetitions&view=participations&filter_search=' . rawurlencode((string) $item->name)); ?>">
            ...
            <?php if (!$this->items) : ?>
                <tr>
                    <td colspan="8" class="text-center py-5 text-muted"><?= Text::_('COM_XDECAROCOMPETITIONS_NO_TEAMS'); ?></td>
                </tr>
            <?php endif; ?>
            </tbody>
        </table>
