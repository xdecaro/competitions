...
?>
<form action="<?= Route::_('index.php?option=com_decarodcl&view=zones'); ?>" method="post" name="adminForm" id="adminForm" class="dcl-admin">
    <div class="dcl-filterbar">
        <div class="dcl-filterbar__search">
            ...
            <input type="search" name="filter_search" id="filter_search" value="<?= $this->escape($this->state->get('filter.search')); ?>" placeholder="<?= Text::_('COM_DECARODCL_SEARCH_ZONES'); ?>">
        </div>

        <select name="filter_organization_id" onchange="this.form.submit()">
            <option value="0"><?= Text::_('COM_DECARODCL_FILTER_ALL_ORGANIZATIONS'); ?></option>
        ...
        <a class="btn btn-secondary" href="<?= Route::_('index.php?option=com_decarodcl&view=zones'); ?>"><?= Text::_('JSEARCH_FILTER_CLEAR'); ?></a>
    </div>
        ...
        <table class="table table-striped align-middle dcl-responsive-table">
            <thead>
                <tr>
                    ...
                    <th><?= HTMLHelper::_('searchtools.sort', 'COM_DECARODCL_FIELD_ORGANIZATION', 'o.name', $listDirn, $listOrder); ?></th>
                    <th class="text-center"><?= HTMLHelper::_('searchtools.sort', 'COM_DECARODCL_COUNTRIES', 'countries_count', $listDirn, $listOrder); ?></th>
                    <th><?= Text::_('COM_DECARODCL_ZONE_COUNTRIES'); ?></th>
            ...
            <?php foreach ($this->items as $i => $item) : ?>
                <tr>
                    ...
                    <td data-label="<?= Text::_('COM_DECARODCL_FIELD_NAME'); ?>">
                        <a class="fw-semibold" href="<?= Route::_('index.php?option=com_decarodcl&task=zone.edit&id=' . (int) $item->id); ?>">
                            <?= $this->escape($item->name); ?>
                        </a>
                    </td>
                    <td data-label="<?= Text::_('COM_DECARODCL_FIELD_CODE'); ?>"><span class="badge bg-secondary"><?= $this->escape($item->code); ?></span></td>
                    <td data-label="<?= Text::_('COM_DECARODCL_FIELD_ORGANIZATION'); ?>">
                        <?= $item->organization_name ? $this->escape($item->organization_name) : Text::_('COM_DECARODCL_ZONE_GLOBAL'); ?>
                    </td>
                    <td class="text-center" data-label="<?= Text::_('COM_DECARODCL_COUNTRIES'); ?>"><?= (int) $item->countries_count; ?></td>
                    <td data-label="<?= Text::_('COM_DECARODCL_ZONE_COUNTRIES'); ?>">
            ...
            <?php if (!$this->items) : ?>
                <tr><td colspan="8" class="text-center py-5 text-muted"><?= Text::_('COM_DECARODCL_NO_ZONES'); ?></td></tr>
            <?php endif; ?>
            </tbody>
        </table>
    </div>
