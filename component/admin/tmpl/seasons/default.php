...
?>
<form action="<?= Route::_('index.php?option=com_decarodcl&view=seasons'); ?>" method="post" name="adminForm" id="adminForm" class="dcl-admin">
    <div class="dcl-filterbar">
        <div class="dcl-filterbar__search">
            ...
            <input type="search" name="filter_search" id="filter_search" value="<?= $this->escape($this->state->get('filter.search')); ?>" placeholder="<?= Text::_('COM_DECARODCL_SEARCH_SEASONS'); ?>">
        </div>
        ...
        <select name="filter_season_year" onchange="this.form.submit()">
            <option value="0"><?= Text::_('COM_DECARODCL_FILTER_ALL_YEARS'); ?></option>
        ...
        <a class="btn btn-secondary" href="<?= Route::_('index.php?option=com_decarodcl&view=seasons'); ?>"><?= Text::_('JSEARCH_FILTER_CLEAR'); ?></a>
    </div>
    ...
    <div class="table-responsive">
        <table class="table table-striped align-middle dcl-responsive-table">
            <thead><tr>
                ...
                <th><?= HTMLHelper::_('searchtools.sort', 'COM_DECARODCL_FIELD_TOURNAMENT', 't.name', $listDirn, $listOrder); ?></th>
                <th><?= HTMLHelper::_('searchtools.sort', 'COM_DECARODCL_FIELD_SEASON_YEAR', 'a.season_year', $listDirn, $listOrder); ?></th>
                <th><?= Text::_('COM_DECARODCL_FIELD_HOST'); ?></th>
                <th><?= HTMLHelper::_('searchtools.sort', 'COM_DECARODCL_FIELD_DATES', 'a.start_date', $listDirn, $listOrder); ?></th>
            ...
            <?php foreach ($this->items as $i => $item) : ?>
                ...
                <tr>
                    ...
                    <td data-label="<?= Text::_('COM_DECARODCL_FIELD_NAME'); ?>"><a class="fw-semibold" href="<?= Route::_('index.php?option=com_decarodcl&task=season.edit&id=' . (int) $item->id); ?>"><?= $this->escape($item->name); ?></a></td>
                    <td data-label="<?= Text::_('COM_DECARODCL_FIELD_TOURNAMENT'); ?>"><?= $this->escape($item->tournament_name ?: '—'); ?><?php if ($item->tournament_code) : ?><div class="small text-muted"><?= $this->escape($item->tournament_code); ?></div><?php endif; ?></td>
                    <td data-label="<?= Text::_('COM_DECARODCL_FIELD_SEASON_YEAR'); ?>"><?= $item->season_year ? (int) $item->season_year : '—'; ?></td>
                    <td data-label="<?= Text::_('COM_DECARODCL_FIELD_HOST'); ?>"><?= $this->escape($host); ?></td>
                    <td data-label="<?= Text::_('COM_DECARODCL_FIELD_DATES'); ?>"><?= $this->escape($dateRange); ?></td>
            ...
            <?php endforeach; ?>
            <?php if (!$this->items) : ?><tr><td colspan="8" class="text-center py-5 text-muted"><?= Text::_('COM_DECARODCL_NO_SEASONS'); ?></td></tr><?php endif; ?>
            </tbody>
        </table>
    </div>
