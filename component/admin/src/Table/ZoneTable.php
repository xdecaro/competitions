...
final class ZoneTable extends Table
{
    ...
    public function check(): bool
    {
        ...
        if ($this->name === '') {
            $this->setError(Text::_('COM_DECARODCL_ERROR_ZONE_NAME_REQUIRED'));
            return false;
        }

        if (!preg_match('/^[A-Z0-9][A-Z0-9_-]{1,49}$/', $this->code)) {
            $this->setError(Text::_('COM_DECARODCL_ERROR_ZONE_CODE_INVALID'));
            return false;
        }
        ...
        if ($this->organization_id > 0) {
            ...
            if ((int) $db->setQuery($query)->loadResult() === 0) {
                $this->setError(Text::_('COM_DECARODCL_ERROR_ZONE_ORGANIZATION_INVALID'));
                return false;
            }
        }
        ...
        if ((int) $db->setQuery($query)->loadResult() > 0) {
            $this->setError(Text::_('COM_DECARODCL_ERROR_ZONE_CODE_DUPLICATE'));
            return false;
        }
