...
final class OrganizationsModel extends ListModel
{
    ...
    protected function getListQuery(): QueryInterface
    {
        ...
        $query = $db->getQuery(true)
            ...
            ->from($db->quoteName('#__dcl_organizations', 'a'))
            ->leftJoin($db->quoteName('#__dcl_countries', 'c') . ' ON ' . $db->quoteName('c.id') . ' = ' . $db->quoteName('a.country_id'));
    ...
    {
        $db = $this->getDatabase();
        $query = $db->getQuery(true)->select([$db->quoteName('id'),$db->quoteName('name'),$db->quoteName('code')])->from($db->quoteName('#__dcl_countries'))->where($db->quoteName('state') . ' <> -2')->order($db->quoteName('name') . ' ASC');
