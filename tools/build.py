...
def normalize_component_schema(stage: Path, manifest: Path) -> None:
    base_sql_path = stage / "admin" / "sql" / "install.mysql.utf8mb4.sql"
    base_sql = base_sql_path.read_text(encoding="utf-8")

    player_column_anchor = "  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,\n  `external_ref` VARCHAR(100) DEFAULT NULL,"
    if "`person_uuid` CHAR(36) NULL" not in base_sql:
        if player_column_anchor not in base_sql:
            raise SystemExit("unable to locate players column anchor for reinstall-safe schema normalization")
        base_sql = base_sql.replace(
            player_column_anchor,
            "  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,\n  `person_uuid` CHAR(36) NULL,\n  `external_ref` VARCHAR(100) DEFAULT NULL,",
            1,
        )

    player_index_anchor = "  PRIMARY KEY (`id`),\n  UNIQUE KEY `idx_competitions_players_external_ref` (`external_ref`),"
    if "UNIQUE KEY `uq_player_person_uuid` (`person_uuid`)" not in base_sql:
        if player_index_anchor not in base_sql:
            raise SystemExit("unable to locate players index anchor for reinstall-safe schema normalization")
        base_sql = base_sql.replace(
            player_index_anchor,
    ...
    roster_column_anchor = "  `role` VARCHAR(50) DEFAULT NULL,\n  `status` VARCHAR(32) NOT NULL DEFAULT 'pending',"
    if "`photo` VARCHAR(512) NULL" not in base_sql:
        if roster_column_anchor not in base_sql:
            raise SystemExit("unable to locate roster column anchor for reinstall-safe schema normalization")
        base_sql = base_sql.replace(
            roster_column_anchor,
    ...
    migration_install = '<file driver="mysql" charset="utf8">sql/install.1.4.0.mysql.utf8mb4.sql</file>'
    manifest_text = manifest_text.replace(migration_install, "")
    manifest.write_text(manifest_text, encoding="utf-8")


def validate_reinstall_safe_schema(stage: Path, manifest: Path) -> None:
    base_sql = (stage / "admin" / "sql" / "install.mysql.utf8mb4.sql").read_text(encoding="utf-8")
    manifest_text = manifest.read_text(encoding="utf-8")
    ...
    if "sql/install.1.4.0.mysql.utf8mb4.sql" in manifest_text:
        raise SystemExit(
...
def stage_extension(source: Path, stage: Path, *, component: bool = False) -> None:
    ...
    normalize_component_schema(stage, manifest)
    validate_reinstall_safe_schema(stage, manifest)
