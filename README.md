...
Version 1.5.13 fixes the confirmed case where the Team Import page rendered but its live-search JavaScript was never attached to the document. The page-specific asset is now registered and used from the rendered Joomla template through Web Asset Manager, preserving live filtering, selections, counters, selected-only review and visible-row select-all.
