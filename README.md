...
Version 1.5.14 fixes the confirmed Team Import Web Asset lifecycle issue by registering the page-specific script before any administrator assets are enabled, then using it only on the Team Import view. This keeps Joomla Web Asset Manager usage while ensuring the browser actually receives the live-search JavaScript.
