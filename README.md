...
Version 1.5.11 makes the Organizations team-import search fully live. Filtering no longer reloads the page, so administrators can search one Club, select it, search another Club and keep building the same multi-selection. The preview also shows selected/visible counters, supports reviewing selected rows only and applies select-all only to currently visible rows.
...
The Teams administrator also provides an Organizations import preview. It consumes the same public provider boundary, loads one stable window of up to 200 Club organizations, marks already-linked UUIDs as non-importable duplicates, shows active sports-affiliation readiness and imports selected Clubs through the normal Team model as Pending records. Search/filtering happens client-side so checkbox selections persist while the administrator searches multiple Club names; select-all applies only to visible rows. No Organizations private table is queried by the import workflow.
